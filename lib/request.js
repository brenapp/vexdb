...
async function request(endpoint, params) {
    ...
    let data = await fetch(cache.serialize(`${constants.settings.baseURL}/get_${endpoint}`, params), {
    ...
    return requestSize(endpoint, params)
        .then(items => Promise.all(
            [...Array(Math.ceil(items / 5000))]
                .map(
                    (v, i) => request(endpoint, Object.assign(params || {}, {
                        limit_start: 5000 * i
                    }))
                )
        ))
...
function paramSort(params, endpoint) {
    ...
    for (let param in params) {
        if (params.hasOwnProperty(param)) {
            ...
            if (typeof value === "string" && constants.validParams[endpoint].includes(param)) {
                out.universal[param] = value;
            } else if (value instanceof Array && constants.validParams[endpoint].includes(param)) {
...
async function get(endpoint, params) {
    ...
    return filter(req
        .filter((e, i, a) => keys.indexOf(JSON.stringify(e)) === i), // Uniqify
        async item =>
            (await Promise.all(
...
async function size(endpoint, params) {
    ...
    if (Object.values(filters).length > 0) {
        ...
        return (await Promise.all(permutationObject(universal, permutations)
            .map(p =>
