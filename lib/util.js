...
const idMap = new Map([
    // events
    [a => a.hasOwnProperty("sku") && a.key === a.sku,
    event => event.sku],
    // teams
    ...
    // awards
    [a => a.hasOwnProperty("qualifies"),
    awards => `${award.sku}<${award.name}>`
    ],
    // season_rankings
    [a => a.hasOwnProperty("vrating"),
    rank => `${rank.team}:${rank.rank}`]
]);

/**
 * Generates a globally unique ID for a given endpoint result
 * @param {Object} item The endpoint result
 */
function key(item) {
    let tests = idMap.keys();
    for (const test of tests) {
        if (test(item)) return idMap.get(test)(item)
    }
}
