...
/**
 * Saves cache.current to its long term storage location (either localStorage, or the cache file)
 */
function save() {
    return new Promise((resolve, reject) => {
        if (isBrowser()) {
            for (const item in cache.current) {
                if (cache.current.hasOwnProperty(item)) {
                    localStorage[`vexdb_${item}`] = cache.current[item]
                }
            }
            resolve();
        } else {
            try {
                const fs = require("fs");
                function saveToDisk() {
                    let old = require(settings.cache.file);
                    for (const item in cache.current) {
                        if (cache.current.hasOwnProperty(item)) {
                            old[item] = cache.current[item]
                        }
                    }
